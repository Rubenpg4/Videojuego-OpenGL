...
igvEscena3D::igvEscena3D() { 
	...
	grLib8 = 0; //grado de libertad 8: el muñeco se agacha pulsando 'm' y se estira pulsando 'M'
...
void igvEscena3D::crearEnemigo() {
	...
	añadirEnemigo(interfaz.getNumOleada(), tipoE, -4, ejey, 0);
	numEnemigos--;
}
	...
	if (interfaz.getEstructuraDisparo() != - 1) {
		...
		while (i != enemigos.end()) {
			if ((abs(i.operator*()->getX() - posicionBala[X]) +
				...
				i.operator*()->decrementarSalud(daño);

				if (i.operator*()->getSalud() <= 0) {
					cout << "enemigos tam: " << enemigos.size() << endl;
...
bool igvEscena3D::comprar(tipoEstructura tipo) {
	...
	if ((terrenos[interfaz.getTerrenoSeleccionado()]->getTipo() == NINGUNA)) {
		if (interfaz.getDinero() >= precioConstruir) {
			añadirEstructura(1, tipo, interfaz.getTerrenoSeleccionado());
	...
	if (terrenos[interfaz.getTerrenoSeleccionado()]->getTipo() == CAÑON) {
		if (myTerreno->getDefensaCañon()->getNivel() < 3) {
			if (interfaz.getDinero() >= myTerreno->getDefensaCañon()->getPrecioMejora()) {
				...
				añadirEstructura(myTerreno->getDefensaCañon()->getNivel() + 1, tipo, interfaz.getTerrenoSeleccionado());
				return true;
			}
		}
	}
		...
		if (myTerreno->getDefensaTorre()->getNivel() < 3) {
			if (interfaz.getDinero() >= myTerreno->getDefensaTorre()->getPrecioMejora()) {
				...
				añadirEstructura(myTerreno->getDefensaTorre()->getNivel() + 1, tipo, interfaz.getTerrenoSeleccionado());
				return true;
			}
		}
	}
...
void igvEscena3D::añadirEnemigo(int nivel, tipoEnemigo _tipo, float _x, float _y, float _z) {
	Enemigo* ene = new Enemigo(nivel, _tipo, _x, _y, _z);
	this->enemigos.push_back(ene);
}

void igvEscena3D::añadirEstructura(int nivel, tipoEstructura _tipo, int nTerreno) {
